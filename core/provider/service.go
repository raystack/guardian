...
func (s *Service) ListAccess(ctx context.Context, providerID string) (*domain.Provider, domain.ResourceAccess, error) {
	...
	c := s.getClient(p.Type)
	resourceAccess, err := c.ListAccess(ctx, *p.Config)
	if err != nil {
		return nil, nil, err
	}

	return p, resourceAccess, nil
}
