...
func PolicyCmd(c *app.CLIConfig, adapter handlerv1.ProtoAdapter) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "policy",
...
func listPoliciesCmd(c *app.CLIConfig) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := client.ListPolicies(ctx, &guardianv1.ListPoliciesRequest{})
			if err != nil {
				return err
			}
...
func getPolicyCmd(c *app.CLIConfig, adapter handlerv1.ProtoAdapter) *cobra.Command {
	var format string
	cmd := &cobra.Command{
		Use:   "view",
		Short: "View a policy",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := client.GetPolicy(ctx, &guardianv1.GetPolicyRequest{
				Id:      id,
...
func createPolicyCmd(c *app.CLIConfig, adapter handlerv1.ProtoAdapter) *cobra.Command {
	var filePath string
	cmd := &cobra.Command{
		Use:   "create",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := client.CreatePolicy(ctx, &guardianv1.CreatePolicyRequest{
				Policy: policyProto,
			})
			if err != nil {
				return err
...
func updatePolicyCmd(c *app.CLIConfig, adapter handlerv1.ProtoAdapter) *cobra.Command {
	var filePath string
	cmd := &cobra.Command{
		Use:   "edit",
		Short: "Edit a policy",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = client.UpdatePolicy(ctx, &guardianv1.UpdatePolicyRequest{
				Id:     policyID,
				Policy: policyProto,
			})
			if err != nil {
