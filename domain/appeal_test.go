...
func TestAppeal_AdvanceApproval(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.appeal.AdvanceApproval(context.TODO()); (err != nil) != tt.wantErr {
...
func TestAppeal_AdvanceApproval_UpdateApprovalStatuses(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			actualError := appeal.AdvanceApproval(context.Background())
...
func TestAppeal_ResolveApprovers(t *testing.T) {
	tests := []struct {
		name        string
		appeal      domain.Appeal
		expressions []string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.appeal.ResolveApprovers(tt.expressions)
			if (err != nil) != tt.wantErr {
				t.Errorf("Appeal.ResolveApprovers() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Appeal.ResolveApprovers() = %v, want %v", got, tt.want)
			}
		})
	}
}
