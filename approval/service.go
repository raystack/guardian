...
func (s *service) AdvanceApproval(appeal *domain.Appeal) error {
	...
	for i, approval := range appeal.Approvals {
		if approval.Status == domain.ApprovalStatusRejected {
			...
			if stepConfig.When != "" {
				v, err := domain.Expression(stepConfig.When).EvaluateWithVars(map[string]interface{}{
					"appeal": appealMap,
				})
				if err != nil {
					return err
			...
			if stepConfig.Strategy == "auto" {
				v, err := domain.Expression(stepConfig.ApproveIf).EvaluateWithVars(map[string]interface{}{
					"appeal": appealMap,
				})
				if err != nil {
					return err
